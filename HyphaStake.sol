...
contract HyphaToken is ERC20, AccessControl {
    
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    
    constructor() ERC20("Hypha Token", "HYPHA") {
...
contract HyphaStake is IERC900, IERC2917, AccessControl, ReentrancyGuard, Pausable {
    
    using SafeMath for uint256;
    
    struct HyphaStakerInfo {
    ...
    mapping(address => HyphaStakerInfo) public stakers;
    ...
    constructor(HyphaToken _HYPHA, uint256 _reward) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    ...
    function stake(uint256 amount, bytes memory data) public override nonReentrant whenNotPaused update {
        ...
        require(HYPHA.transferFrom(_msgSender(), address(this), amount));
        
        updateCheckpointAtNow(stakesFor[_msgSender()], amount, false);
        updateCheckpointAtNow(stakeHistory, amount, false);
        
        HyphaStakerInfo storage staker = stakers[_msgSender()];
    ...
    function unstake(uint256 amount, bytes memory data) public override nonReentrant whenNotPaused update {
        ...
        require(HYPHA.transfer(_msgSender(), amount));
    ...
    function takeWithAddress(address user) external view returns (uint256) {
        HyphaStakerInfo storage staker = stakers[user];
        uint256 _accAmountPerShare = accAmountPerShare;
    ...
    function claim() public nonReentrant whenNotPaused update {
        ...
        if(_pending > 0) {
            ...
            HYPHA.mint(_msgSender(), _pending);
